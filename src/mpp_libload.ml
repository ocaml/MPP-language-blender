...
let html_of_ocaml filename cs out =
  let ic = open_in filename in
    ...
    Out.printf out "<pre class='ocaml'>\n";
    try
...
let html_of_ocaml_default_css _ _ out =
  Out.printf out "<style type='text/css'>/* <!-- */
...
(*         let () = Mpp_actions.cat out tmp in *)
        ...
        let () = Out.printf out "<pre class='ocaml'>" in
...
let () =
  Mpp_actions.register "ocamlhtmlcss" (html_of_ocaml_default_css) "Prints a CSS for ocamlhtml.";
  Mpp_actions.register "ocamlhtml" (html_of_ocaml) "OCaml syntax highlighting (outputs HTML).";
  Mpp_actions.register "ocamlcerror" (html_of_ocamlc) "Execute an ocamlc or ocamlopt command, and process (i.e. turn it into HTML) the output error message if any.";
